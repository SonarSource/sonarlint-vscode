...
export class FlightRecorderService {
  ...
  async onFlightRecorderStarted(sessionId: string) {
    ...
    const flightRecorderStatusItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, FLIGHT_RECORDER_ITEM_PRIORITY);
    flightRecorderStatusItem.text = '$(record) SonarQube Flight Recorder';
    flightRecorderStatusItem.command = Commands.SHOW_FLIGHT_RECORDING_MENU;
    flightRecorderStatusItem.tooltip = `SonarQube Flight Recorder session '${this.sessionId}' running.
Click to copy session ID or capture a thread dump.`;
    flightRecorderStatusItem.show();

    const copySelected = await vscode.window.showInformationMessage(`SonarQube Flight Recorder started with session ID '${this.sessionId}'.`, 'Copy Session ID');
    if (copySelected) {
