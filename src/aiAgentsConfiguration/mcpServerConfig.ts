...
async function getStandaloneMCPAgents(languageClient: SonarLintExtendedLanguageClient) {
...
export async function refreshStandaloneMCPAgentContext(languageClient: SonarLintExtendedLanguageClient): Promise<void> {
  try {
    await getStandaloneMCPAgents(languageClient);
  } catch (error) {
    logToSonarLintOutput(`Could not detect agents supporting standalone MCP: ${String(error)}`);
  }
}
