...
function mcpFailed(
  ...
  return { status: AiIntegration.AiIntegrationActionStatus.FAILED, failureCategory, agent };
}
...
function blockedMcpOutcome(state: AiIntegration.McpConfigurationState, agent: AiIntegration.AiAgent): AiIntegrationOutcome {
  return mcpFailed(
    state === AiIntegration.McpConfigurationState.MALFORMED
      ? AiIntegration.AiIntegrationFailureCategory.MALFORMED_CONFIGURATION
      : AiIntegration.AiIntegrationFailureCategory.UNSUPPORTED,
    agent
  );
}
...
function unselectedMcpAgentOutcome(
  ...
  return selection.kind === 'cancelled'
    ? mcpCancelled(requestedAgent)
    : mcpFailed(AiIntegration.AiIntegrationFailureCategory.UNSUPPORTED, requestedAgent);
}
...
function unselectedMcpConnectionOutcome(
  ...
  return selection.kind === 'cancelled'
    ? mcpCancelled(agent)
    : mcpFailed(AiIntegration.AiIntegrationFailureCategory.CONNECTION_UNAVAILABLE, agent);
}
...
export async function configureMCPServer(
  ...
  if (vscode.env.remoteName !== undefined) {
    ...
    return mcpFailed(AiIntegration.AiIntegrationFailureCategory.UNSUPPORTED, requestedAgent);
  }
  ...
  let failureCategory = AiIntegration.AiIntegrationFailureCategory.UNKNOWN;
  try {
    failureCategory = AiIntegration.AiIntegrationFailureCategory.FILESYSTEM_ERROR;
    ...
    if (!isUpdateAllowed(inspection.state)) {
      ...
      return blockedMcpOutcome(inspection.state, agent);
    }
    ...
    if (!isUpdateAllowed(updatePlan.state) || updatePlan.updatedContent == null) {
      ...
      return blockedMcpOutcome(updatePlan.state, agent);
    }
    ...
    return mcpFailed(failureCategory, selectedAgent);
  } finally {
...
async function selectMCPAgent(
  ...
  if (agents.length === 0) {
    await vscode.window.showInformationMessage('No detected agent supports standalone MCP setup.');
    return { kind: 'unsupported' };
  }
  if (agents.length === 1) {
...
export async function openMCPServerConfigurationFile(
  ...
  let failureCategory = AiIntegration.AiIntegrationFailureCategory.BACKEND_ERROR;
  try {
    const selection = await selectMCPAgent(languageClient, requestedAgent);
    if (selection.kind !== 'selected') {
      return selection.kind === 'cancelled'
    ...
    if (!fs.existsSync(configPath)) {
      ...
      return mcpFailed(AiIntegration.AiIntegrationFailureCategory.FILESYSTEM_ERROR, agent);
    }
    await vscode.window.showTextDocument(vscode.Uri.file(configPath));
    return { status: AiIntegration.AiIntegrationActionStatus.SUCCEEDED, agent };
  } catch {
    return mcpFailed(failureCategory, requestedAgent);
  }
}
