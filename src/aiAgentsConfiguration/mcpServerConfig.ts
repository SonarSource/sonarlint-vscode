...
export function hasActiveStandaloneMcpAgent(): boolean {
  return getDetectedIdeAgents().some(agent => isStandaloneMcpReady(agent.id));
}
...
async function getStandaloneMCPAgents(languageClient: SonarLintExtendedLanguageClient) {
  ...
  return getDetectedIntegrationAgents(state).filter(agent => agent.standaloneMcpSupported && isStandaloneMcpReady(agent.agent));
}
