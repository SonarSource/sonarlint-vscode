  ...
  private getFolders(): FolderOption[] {
    ...
    return (VSCode.workspace.workspaceFolders ?? []).map(folder => {
      ...
      return {
        ...
        isStandalone: !(bindingState.get(folder.uri) ?? false)
      };
    });
  }

  public static async showSupportedLanguages(context: VSCode.ExtensionContext, languageClient: SonarLintExtendedLanguageClient) {
  ...
  private static create(context: VSCode.ExtensionContext, languageClient: SonarLintExtendedLanguageClient) {
    ...
    panel.webview.onDidReceiveMessage(
      msg => {
        if (msg.command === 'setupConnection') {
        ...
        } else if (msg.command === 'bindProject') {
          AutoBindingService.instance.getTargetConnectionForManualBinding().then(targetConnection => {
            if (targetConnection) {
              const folder = VSCode.workspace.workspaceFolders?.find(
                f => code2ProtocolConverter(f.uri) === instance.selectedScopeId
              );
              BindingService.instance.createOrEditBinding(
                targetConnection.connectionId,
                targetConnection.contextValue,
                folder
              );
            }
          });
          ...
          instance.fetchAndShow(msg.configScopeId);
        }
      },
      null,
...
function renderTable(statuses: ExtendedServer.PluginStatusDto[]): string {
  ...
  return `<table class="plugins-table">
      <thead>
        <tr>
          <th aria-sort="none"><button class="sort-btn" data-col="0">ANALYSIS TYPE<span class="sort-indicator" aria-hidden="true"></span></button></th>
