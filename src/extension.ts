...
function computeRuleDescPanelContent(context: VSCode.ExtensionContext, ruleKey:string, ruleName:string, htmlDesc:string, ruleType:string, ruleSeverity:string) {
	let severityImg = Path.resolve(context.extensionPath, "images", "severity", ruleSeverity.toLowerCase() + ".png");
	let typeImg = Path.resolve(context.extensionPath, "images", "type", ruleType.toLowerCase() + ".png");
	return `<!doctype html><html>
		...
		<div>
		<img style="padding-bottom: 1px;vertical-align: middle" width="16" height="16" alt="${ruleType}" src="data:image/gif;base64,${base64_encode(typeImg)}">&nbsp;
		${clean(ruleType)}&nbsp;
		<img style="padding-bottom: 1px;vertical-align: middle" width="16" height="16" alt="${ruleSeverity}" src="data:image/gif;base64,${base64_encode(severityImg)}">&nbsp;
		${clean(ruleSeverity)}
		</div>
