...
describe('CFamily Test Suite', () => {
  ...
  test('Compile database autosuggestion', async () => {
    ...
    await sleep(300000);
    await vscode.commands.executeCommand('workbench.action.acceptSelectedItem');

    const settings = await vscode.workspace.getConfiguration('sonarlint', projectUri)
      .get('pathToCompileCommands', vscode.ConfigurationTarget.WorkspaceFolder);
