...
suite('Connected Mode Test Suite', () => {
  beforeEach(async () => {
    ...
    await VSCode.workspace.getConfiguration(SONARLINT_CATEGORY)
    .update(CONNECTED_MODE_SETTINGS_SONARQUBE, [TEST_SONARQUBE_CONNECTION], VSCode.ConfigurationTarget.Global);
    
    // remove all existing bindings
    VSCode.workspace.workspaceFolders.forEach(async (folder) => {
      await VSCode.workspace.getConfiguration(SONARLINT_CATEGORY, folder.uri)
  ...
  describe('Bindings Manager', () => {
    ...
    test('Save binding updates configuration', async () => {
      ...
      const existingBinding =  VSCode.workspace.getConfiguration(SONARLINT_CATEGORY, workspaceFolder.uri)
      .get(BINDING_SETTINGS);
      expect(existingBinding).to.be.empty;
      
      underTest.saveBinding(TEST_BINDING.projectKey, TEST_BINDING.connectionId, workspaceFolder)
      .then(_a => {
        updatedBinding = VSCode.workspace.getConfiguration(SONARLINT_CATEGORY, workspaceFolder.uri)
        .get(BINDING_SETTINGS);
    ...
    test('Folder selection QuickPick should directly return folder name when only 1 folder in WS', async () => {
      ...
      await underTest.showFolderSelectionQuickPickOrReturnDefaultSelection(workspaceFolders);
