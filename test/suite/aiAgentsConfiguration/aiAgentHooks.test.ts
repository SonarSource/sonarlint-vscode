...
import { IntegrationTarget, getCurrentAgentWithHookSupport } from '../../../src/aiAgentsConfiguration/aiAgentUtils';
...
suite('aiAgentHooks', () => {
  ...
  suite('isHookInstalled', () => {
    test('should return false when agent is CURSOR (not supported)', async () => {
      const result = await isHookInstalled(IntegrationTarget.CURSOR);
    ...
    test('should return false when hooks.json does not exist', async () => {
      ...
      const result = await isHookInstalled(IntegrationTarget.WINDSURF);
  ...
  suite('installHook', () => {
    ...
    test('should show error for unsupported agent', async () => {
      await installHook(mockLanguageClient, IntegrationTarget.CURSOR);
    ...
    (process.platform === 'win32' ? test.skip : test)('should install hook successfully for Windsurf when no existing hooks', async () => {
      ...
      await installHook(mockLanguageClient, IntegrationTarget.WINDSURF);
  ...
  suite('uninstallHook', () => {
    test('should show error for unsupported agent', async () => {
      await uninstallHook(IntegrationTarget.CURSOR);
    ...
    test('should show info message when no hook is installed', async () => {
      ...
      await uninstallHook(IntegrationTarget.WINDSURF);
  ...
  suite('openHookScript', () => {
    test('should show error for unsupported agent', async () => {
      await openHookScript(IntegrationTarget.CURSOR);
    ...
    test('should show info message when no hook is found', async () => {
      ...
      await openHookScript(IntegrationTarget.WINDSURF);
  ...
  suite('openHookConfiguration', () => {
    test('should show error for unsupported agent', async () => {
      await openHookConfiguration(IntegrationTarget.CURSOR);
    ...
    test('should show info message when configuration file does not exist', async () => {
      ...
      await openHookConfiguration(IntegrationTarget.WINDSURF);
