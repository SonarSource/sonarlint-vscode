...
suite('mcpServerConfig', () => {
  ...
  test('should normalize a connection without an ID before generating and persisting its configuration', async () => {
    ...
    try {
      ...
      expect(inspectMcpConfigurationStub.firstCall.args[0].content).to.equal(null);
      expect(planMcpConfigurationUpdateStub.firstCall.args[0].content).to.equal(null);
      expect(planMcpConfigurationUpdateStub.firstCall.args[0].sonarMcpConfiguration).to.equal(
        '{"command": "test-command", "args": ["test-arg"], "env": {}}'
      );
  ...
  test('should copy a backup only when one does not already exist', () => {
    const fs = require('node:fs');
    ...
    try {
      backupMcpConfig('/tmp/mcp.json');
      backupMcpConfig('/tmp/mcp.json');
      expect(copyFileStub.calledOnceWith('/tmp/mcp.json', '/tmp/mcp.json.bak')).to.be.true;
    } finally {
      existsStub.restore();
      copyFileStub.restore();
    }
