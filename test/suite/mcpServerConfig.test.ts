...
import { getCurrentAgentSupportedIDE, getMCPConfigPath, IDE, configureMCPServer, onEmbeddedServerStarted } from '../../src/aiAgentsConfiguration/mcpServerConfig';
...
suite('mcpServerConfig', () => {
  test('should detect supported IDEs based on app name', () => {
    ...
    try {
      ...
      expect(getCurrentAgentSupportedIDE()).to.equal(IDE.CURSOR);

      envStub.value('Windsurf');
      expect(getCurrentAgentSupportedIDE()).to.equal(IDE.WINDSURF);

      envStub.value('Visual Studio Code');
      extensionsStub.withArgs('GitHub.copilot').returns({ isActive: true });
      expect(getCurrentAgentSupportedIDE()).to.equal(IDE.VSCODE);

      envStub.value('Visual Studio Code');
      extensionsStub.withArgs('GitHub.copilot').returns({ isActive: false });
      expect(getCurrentAgentSupportedIDE()).to.be.undefined;

      envStub.value('Visual Studio Code - Insiders');
      extensionsStub.withArgs('GitHub.copilot').returns({ isActive: true });
      expect(getCurrentAgentSupportedIDE()).to.equal(IDE.VSCODE_INSIDERS);
